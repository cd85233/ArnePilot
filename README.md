...
- [Dynamic distance profiles](https://github.com/ShaneSmiskol/ArnePilot/tree/stock_additions-devel#dynamic-follow-3-profiles) from Shane (In other word three different dynamic profiles: (`close`, `normal`, `far`, `auto`). Profile can be adjusted from either op_edit.py or using the on screen buttons(can take up to 4 sec to for new profile to be adjusted). NOTE: To enable the GUI selection first, Go to setting top left. Click on dragon pilot and then control button on top and then scroll down a bit then their is + - to turn on amd off. Once it’s on it can be changed from op ui.
